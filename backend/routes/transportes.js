...
router.post("/", async (req, res) => {
  try {
    const { vehiculo, conductor, disponibilidad, lat, lng } = req.body;
    ...
    const result = await db.run(
      ...
      [vehiculo, conductor ?? null, disp, lat ?? null, lng ?? null]
    );
...
router.put("/:id", async (req, res) => {
  try {
    ...
    await db.run(
      "UPDATE transportes SET vehiculo = ?, conductor = ?, disponibilidad = ?, lat = ?, lng = ? WHERE id = ?",
      [vehiculo, conductor ?? null, disp, lat ?? null, lng ?? null, id]
    );
